...
// Utils
import { neo4jQuery } from '../db';
import { getFirstLetters, getWords } from '../../utils';
import { PROMOTION_GROUP_CREATED } from './subscriptionTypes';
...
const Mutation = {
  ...
  async UpdateCompanyAccount(parent, params, ctx, resolveInfo) {
    try {
      ...
      throw Error(`Error encountered while updating access group: ${e}`);
    }
  },
  ...
  async CreateStore(parent, params, ctx, resolveInfo) {
    const { companyId } = ctx.user;
    const { store, account, location } = params;
  ...
  async CreateProductGroup(parent, params, ctx, resolveInfo) {
    const { storeId, productGroup, files } = params;
  ...
  async UpdateProductGroup(parent, params, ctx, resolveInfo) {
    const { storeId, groupId, productGroup, files } = params;
    ...
    try {
      ...
      if (!result) {
        throw Error(
          `Product Group with name '${productGroup.name}' not found.`
        );
      } else {
        let products = [];
  ...
  async UpdateProduct(parent, params, ctx, resolveInfo) {
    ...
    const { productId, productInput } = params;

    try {
      if (
        !productInput.name ||
        !productInput.shortDescription ||
        !productInput.longDescription ||
        !productInput.brand ||
        !productInput.price ||
        !productInput.stock
      ) {
        throw Error(
          `Missing product inputs, required inputs: name, shortDescription, longDescription, brand, price, stock. Received data: ${JSON.stringify(
            Object.keys(productInput)
          )}.`
        );
      } else {
        const data = {
  ...
  async CreatePromotionGroup(parent, params, ctx, resolveInfo) {
    ...
    try {
      ...
      if (result) {
        throw Error(
          `Promotion Group with name '${input.name}' already exists.`
        );
      } else {
        const data = {
          storeId,
  ...
  async LinkPromotionGroupToProductGroups(parent, params, ctx, resolveInfo) {
    try {
      ...
      console.error(
        `Error while linking promotion group to product groups: ${e}`
      );
    }
  },
